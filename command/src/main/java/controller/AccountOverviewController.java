...
public class AccountOverviewController {
    ...
    private void handleDeleteAction(ActionEvent event) {
        commandRegistry.executeCommand(new RemoveTransactionsCommand(data,
                transactionsTable.getSelectionModel().getSelectedItems().stream().toList()
        ));
    }
    ...
    private void handleAddAction(ActionEvent event) {
        ...
        if (appController.showTransactionEditDialog(transaction)) {
            AddTransactionCommand command = new AddTransactionCommand(data, transaction);
